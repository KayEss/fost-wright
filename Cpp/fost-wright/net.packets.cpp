...
namespace {
    fostlib::performance
            p_out_version(wright::c_exec_helper, "network", "out", "version");
    fostlib::performance
            p_in_version(wright::c_exec_helper, "network", "in", "version");
}
fostlib::hod::out_packet wright::out::version(capacity &cap) {
    const std::size_t total_capacity =
            cap.size() + c_overspill_cap_per_worker.value() * cap.children();
    ++p_out_version;
...
void wright::in::version(
    ...
    logger("version", "packet",
           version)("version", "protocol", g_proto.max_version())(
            "version", "old",
            old_version)("version", "negotiated", cnx->version());
    ...
    if (packet.size()) {
        ...
        if (cnx->peer == connection::server_side) {
            logger("", "Version packet processed -- adding capacity")(
                    "capacity", "remote",
                    capacity)("capacity", "local", "old", cnx->capacity.size());
            cnx->capacity.additional(cnx, capacity);
            logger("capacity", "local", "new", cnx->capacity.size());
        } else {
            logger("",
                   "Version packet processed")("capacity", "remote", capacity);
        }
    } else {
...
namespace {
    fostlib::performance
            p_out_execute(wright::c_exec_helper, "network", "out", "execute");
    fostlib::performance
            p_in_execute(wright::c_exec_helper, "network", "in", "execute");
}
...
namespace {
    fostlib::performance p_out_completed(
            wright::c_exec_helper, "network", "out", "completed");
    fostlib::performance
            p_in_completed(wright::c_exec_helper, "network", "in", "completed");
}
...
namespace {
    fostlib::performance p_out_log_message(
            wright::c_exec_helper, "network", "out", "log_message");
    fostlib::performance p_in_log_message(
            wright::c_exec_helper, "network", "in", "log_message");
}
fostlib::hod::out_packet
        wright::out::log_message(const fostlib::log::message &m) {
    ++p_out_log_message;
    auto msg = fostlib::json::unparse(fostlib::coerce<fostlib::json>(m), false);
    fostlib::hod::out_packet packet{packet::log_message};
    packet << fostlib::coerce<fostlib::utf8_string>(msg);
    return packet;
}
void wright::in::log_message(
        std::shared_ptr<connection> cnx, fostlib::hod::tcp_decoder &packet) {
    ++p_in_log_message;
    auto msg = fostlib::hod::read<fostlib::utf8_string>(packet);
    fostlib::log::log(fostlib::log::message(
            cnx->reference,
            fostlib::json::parse(fostlib::coerce<fostlib::string>(msg))));
}
