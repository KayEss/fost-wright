...
int main(int argc, char *argv[]) {
        ...
        return [&](auto &&... a) {
            ...
            for (std::size_t arg{1}; arg != args.size(); ++arg) {
                std::cerr << "Loading config "
                          << fostlib::json(args[arg].value());
                auto filename = fostlib::coerce<boost::filesystem::path>(
                        args[arg].value());
            ...
            fostlib::log::info(
                    ...
                    "Everything done. Terminating normally");
            fostlib::log::flush();
        };
    };
    ...
    /// * `[](){} ` An empty lambda in effect saying "don't re-throw the
    /// exception"
